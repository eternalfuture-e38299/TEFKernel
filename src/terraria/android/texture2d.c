...
static patch_handle_t create_unity_texture2d(const int width, const int height, const texture_format_t texture_format, const void* data, const size_t data_size) {
    ...
    ((void(*)(void*, int, int, int, bool))patchlib_method_get_pointer(unity_texture2d_create))(unity_texture2d, width, height, (int)texture_format, false);
    ...
    void* pixel_data = ((void*(*)(void*, int))patchlib_method_get_pointer(unity_texture2d_get_writable_image_data))(unity_texture2d, 0);
    const uint64_t pixel_data_size = ((uint64_t(*)(void*))patchlib_method_get_pointer(unity_texture2d_get_raw_image_data_size))(unity_texture2d);

    if (pixel_data_size != data_size || pixel_data)
        return PATCH_NULL;
...
patch_handle_t terraria_texture2d_create(const int width, const int height, const texture_format_t texture_format, void* data, const size_t data_size) {
    if (unity_texture2d_class == PATCH_NULL) {
        init_method();
    }
